import '../../scss/blocks/pagination/pagination.scss';

export default function initPagination(selector, data) {
...
function transformNav(state, pagination) {
    ...
    if ((state.size - state.currentPage) < 2) {
        ...
        pagination.querySelector('.page:not(.page_next):not(.page_prev)').insertAdjacentElement('afterend', ellipsis);
        pagination.querySelector('.page:not(.page_next):not(.page_prev)').classList.add('page_first');
        ...
        pagination.querySelector('.ellipsis').remove();
        pagination.querySelectorAll('.page:not(.page_next):not(.page_prev)')[3].insertAdjacentElement('beforebegin', ellipsis);
        pagination.querySelectorAll('.page:not(.page_next):not(.page_prev)')[3].classList.add('page_last');
...
function initState(state, pagination) {
    ...
    state.visibleArray.forEach((element, i) => {
        pagination.querySelectorAll('.page:not(.page_first):not(.page_last):not(.page_next):not(.page_prev)')[i].textContent = element;
    });

    pagination.querySelectorAll('.page:not(.page_next):not(.page_prev)').forEach(element => {
    ...
    text.textContent = `${state.currentPage*12 - 11} - ${state.currentPage*12} из 100+ вариантов аренды`
